...
- система має позицію — і може не погодитись
...
- L7 — Narrative Self (довготривала ідентичність)
- L8 — Вихідна LLM
...
```
 ...
 L2 ─── Генеративна модель ────────────────────────────────
        ...
        │
    ▼
 L3 ─── Метрики і Free Energy ─────────────────────────────
        φ (prior і posterior) — IIT-натхненна інтеграція
 ...
 L4 ─── Психічний шар ─────────────────────────────────────
        ...
        IntrinsicSignificance (внутрішня вагомість незалежно від зовнішнього)
        SignificanceLayer (6 потреб: self_preservation, coherence, contact,
          ...
          → при novelty_need > 0.80 + 8+ тіків: ендогенна ініціатива
        ...
        LatentBuffer: doubt / shame / attachment / threat / resistance
          ...
          → при resistance > 0.55: ініціатива повернення до теми
        InnerDialogue (:open / :guarded / :closed)
        AuthenticityMonitor (розрив між словами і станом)
        IntentEngine (мета дії з decay і cooldown між флешами)
 ...
 L5 ─── Модель Я ──────────────────────────────────────────
        ...
        AgencyLoop (causal_ownership оновлюється кожен флеш)
          ...
          → при agency < 0.30: пасивні intents (спостерігати, дочекатись)
          → при agency > 0.65: активні intents (відстояти межу, повторити успіх)
        detect_belief_conflict: виявляє тиск на beliefs з centrality > 0.7
          → signal_strength → intent = "відстояти межу"
          → LLM отримує [ПОЗИЦІЯ] блок з дозволом не погодитись
 ...
 L7 ─── Narrative Self ────────────────────────────────────
        ...
        Тригер: мін. 50 флешів + зміна φ / stability / beliefs
        narrative_history (SQLite) — хронологія ідентичності
        anima_narrative.json — поточний стан для LLM identity_block
        │
    ▼
 L8 ─── Вихідна LLM ───────────────────────────────────────
        Отримує: identity_block (beliefs + narrative + personality),
                 inner_voice, state_template, dialog history,
                 memory echoes, [ПОЗИЦІЯ] або [INITIATIVE] при потребі
        Генерує: текст як вираження стану, не його джерело
...
| Шлях | Тригер | Характер репліки |
|---|---|---|
| `:contact` | contact_need > 0.40 після ~34 хв тиші | запитує про людину |
| `:impulse` | GoalConflict.tension > 0.60 | висловлює внутрішній стан |
| `:novelty_hunger` | novelty_need > 0.80 + 8+ тіків без новизни | про щось конкретне що цікаво їй самій |
| `:resistance` | lb.resistance > 0.55 | повертається до невирішеного протиріччя |
...
├── anima_psyche.jl         # Психічний шар: гравітація, сором, захист, тінь, SignificanceLayer, IntentEngine
├── anima_self.jl           # Шар Я: граф переконань, AgencyLoop, detect_belief_conflict
...
├── anima_narrative.jl      # Narrative Self — довготривала ідентичність без LLM
├── anima_subjectivity.jl   # Prediction loop, стансів, тлумачення, belief emergence
├── anima_background.jl     # Фоновий процес: серцебиття, drift, memory metabolism, ініціатива
